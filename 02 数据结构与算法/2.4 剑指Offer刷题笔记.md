...
主串为A，模式串为B，A的长度为m，B的长度为n

根据B的最后一个字符是谁，可能是./*/正常字符，进行分类讨论

1. 如果B的最后1个字符是正常字符，则看`A[m - 1]`与B`[n - 1]`是否匹配，若匹配，则看`A[m - 2]`是否与`B[n - 2]`匹配，这是子问题
2. 如果B的最后1个字符是`.`，则看A[m - 2]与B[n - 2]是否匹配，同1
3. 如果B的最后一个字符是`*`，则它修饰前1个字符。表示B[n - 2] = c可以重复0次或者多次，它们是一个整体c*
   - 主串重复0个c, 能否匹配就看`A[m - 1]`与`B[n - 3]`
   - 主串重复多个c, 这种情况下`A[n - 1]`必须等于c，或者`c = .`， A[n - 1]匹配完往前挪，继续看A[m - 2]与B[n - 2]

状态转移方程

```plaintext
dp[i][j]表示A的前i个字符和B的前j个字符能否匹配
对于1和2，
dp[i][j] = dp[i - 1][j - 1] && (A[i] == B[j] || B[j] == '.')
对于3，
...
```
        ...
        转移方程
        f[i][j]表示A的前i个字符[0, i -1]和B的前j个字符[0, j - 1]能否匹配
        对于1和2， f[i][j] = f[i - 1][j - 1]
        对于3
            主串重复0个c, f[i][j] = f[i][j - 2]
            主串重复多个c, f[i][j] = 

不愧是困难题，一个小时也想不明白，暂时放弃
