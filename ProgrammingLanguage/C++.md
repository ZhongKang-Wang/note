...
![image-20250307205055762](C++.assets/image-20250307205055762.png)
...
两个函数会被编译器自动调用，完成对象初始化和清理工作。对象的初始化和清理是编译器强制我们要做的事情，因此如果我们不提供构造和析构，编译器会提供构造函数和析构函数。
...
{
	//test01(); 
	//如果你不写构造函数，编译器会给你自动写一个，如Person(){},是空实现，只会在初始化时调用一次
	//如果你写了构造函数，它就用你的
